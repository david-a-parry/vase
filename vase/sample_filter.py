...
class GtFilter(object):
    ...
    __slots__ = ['gq', 'dp', 'max_dp', 'het_ab', 'hom_ab', 'gt_is_ok',
                 'ab_filter', 'ref_ab_filter', 'ad_over_threshold',
    ...
    def _is_het_or_hom(self, gts, sample, allele):
        ...
        if self.ignore_gts:
            ...
            sgt = gts[sample]['GT']
        if sgt == (allele, allele) or sgt == (allele,):
            is_hom_alt = True
        elif allele in sgt:
            is_het_alt = True
...
class SvGtFilter(GtFilter):
    ...
    __slots__ = ['gq', 'dp', 'max_dp', 'het_ab', 'hom_ab', 'gt_is_ok',
                 ...
                 'get_allele_depths', 'ignore_gts', 'min_support']

    def __init__(self, vcf, gq=0, dp=0, max_dp=0, het_ab=0., hom_ab=0.,
                 ref_ab_filter=None, del_dhffc=None, dup_dhbfc=None,
                 min_support=0, ignore_gts=False):
        '''
            Args:
                vcf:    Input VCF, which will be checked to ensure any
                        specified filters can be used by ensuring
        ...
        if het_ab or hom_ab or ref_ab_filter or min_support or ignore_gts:
            ...
            if ab_fields == ('PR', 'SR'):
                ...
                self.get_allele_depths = self._get_pr_sr
                self.ab_filter = self._ab_filter
    ...
    def _check_header_fields(self, vcf):
        ...
        if (self.dp or self.max_dp or self.het_ab or self.hom_ab
                or self.ref_ab_filter or self.min_support):
